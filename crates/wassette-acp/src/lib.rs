...
mod shared_grants_tests {
    ...
    #[tokio::test]
    async fn policy_free_chains_work_but_privileged_chains_require_opt_in() {
        ...
        let stages = [("provider", &denied), ("layer", &denied)];
        assert!(
            require_shared_grants_opt_in(true, false, stages, &secrets)
                .await
                .is_ok()
        );
