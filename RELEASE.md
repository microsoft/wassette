...
The [Release Doctor workflow](.github/workflows/release-doctor.md) automatically monitors the entire release pipeline and creates diagnostic issues when failures occur.

### What It Monitors

The Release Doctor monitors three critical workflows:

1. **Prepare Release** (`prepare-release.yml`): Version bump PR creation
2. **Release** (`release.yml`): Binary builds, GitHub release creation, CHANGELOG updates
3. **Update Package Manifests** (`update-package-manifests.yml`): Homebrew and WinGet updates

### What It Checks

When a release workflow completes, the Release Doctor performs comprehensive verification:

**For All Workflows:**
- Analyzes failed jobs and extracts error logs
- Identifies root causes and error patterns
- Searches for similar historical failures
- Creates detailed diagnostic issues with remediation steps

**For Release Workflow Specifically:**
- **Binary Verification**: Ensures all 6 platform binaries are present in the GitHub release (Linux/macOS/Windows for AMD64/ARM64)
- **CHANGELOG Synchronization**: Verifies that CHANGELOG.md is properly updated on both main and release branches
- **Version Consistency**: Checks that Cargo.toml version matches the release tag
- **Package Manifest Status**: Confirms that the update-package-manifests workflow was triggered and created its PR

### How It Works

1. **Automatic Triggering**: The workflow automatically runs when any of the three release workflows completes
2. **Smart Detection**: Only investigates failures - if a workflow succeeds, the doctor exits immediately without creating issues
3. **Deep Analysis**: Uses GitHub APIs to inspect workflow runs, job logs, artifacts, and repository state
4. **Issue Creation**: Creates a single comprehensive issue with:
   - Failure summary and root cause analysis
   - Release integrity status report
   - Error log excerpts
   - Specific remediation steps
   - Historical context

### Viewing Release Doctor Results

- **Successful Releases**: No issues are created, check the [workflow runs](https://github.com/microsoft/wassette/actions/workflows/release-doctor.lock.yml) to see verification logs
- **Failed Releases**: Look for issues with the `[release-doctor]` prefix and labels: `release`, `automated`, `ci-failure`
- **Historical Patterns**: The doctor uses memory caching to build knowledge of recurring issues

### Benefits

- **Immediate Notification**: Get detailed diagnostics as soon as a release workflow fails
- **Comprehensive Analysis**: Single issue covers all aspects of the failure
- **Actionable Guidance**: Specific commands and steps to fix the issue
- **No Manual Monitoring**: Automated monitoring eliminates need to watch workflow runs
- **Pattern Recognition**: Learns from past failures to provide better diagnostics
