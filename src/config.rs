...
mod tests {
    ...
    fn test_config_file_not_exists_succeeds_with_defaults() {
        ...
        assert_eq!(config.policy_file, Some(PathBuf::from("test_policy.yaml")));
    }
...
policy_file = "config_policy.yaml"
"#;
...
"#;
        ...
        assert_eq!(
            config.policy_file,
            Some(PathBuf::from("config_policy.yaml"))
        );
    }
...
"#;
        ...
        assert_eq!(config.plugin_dir, PathBuf::from("/config/plugin/dir"));
        // policy_file should come from CLI defaults since not in config file
...
"#;
        ...
        assert_eq!(
            config.policy_file,
            Some(PathBuf::from("custom_policy.yaml"))
        );
    }
}
